...
<?php
	if(isset($_POST['submit']))
	{
		...
		$t=$_POST['type'];
		$s=$_POST['size'];
		$sql = "SELECT * FROM kitches where type='".$t."' AND size='".$s."'" ;//select data from kitchen table
...
?>
<h1 style="color:#0CF;">Kitchen Tiles</h1>
<?php

$sql="SELECT * FROM kitches";// select all data from kitchen table
...
//to show data in the tablle
    ...
    if ($count==0)// If count= 0 it oprn the row
	{ 
				echo "<tr>";
	}
    if ($count==2)//After count=2 it close the row
	{
		$count=0;
		$rowClose=true;
	}
	if($count<4)//If count lessthan 4 it display output	
