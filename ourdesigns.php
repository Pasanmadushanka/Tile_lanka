...
<title>Tile lanka designs</title>
...
<?php
	if(isset($_POST['submit']))
	{
		...
		$t=$_POST['type'];
		$s=$_POST['size'];
		$sql = "SELECT * FROM ourdesign where type='".$t."' AND size='".$s."'" ;//select data from ourdesign table
		$result = mysqli_query($conn,$sql);//declare variable for results
		  if (mysqli_num_rows($result)>0)
		   ...
		   {
			 echo"<b style='color: Red'>Not found '".$s."'  size '".$t."'  tiles</b> ";
		   }
	}
?>
<h1 style="color:#0CF;"><center>Our designs</center></h1>
<?php

$sql="SELECT * FROM ourdesign";// select all data from our design table
