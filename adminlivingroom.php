...
<center>
<br />
<h3>Livingroom tile modifications</h3>
<?php
//declare variables
$server='localhost';
$user='root';
...
<!--Form for moadify living room tiles-->
...
$sql="INSERT INTO livingroom VALUES(".$id.",'".$type."','".$name."','".$size."','".$price."','".$image."')";/*insert data to database livingroom table */
    if(mysqli_query($conn,$sql))
     $msg= "1 row inserted successfully";
else 
     $msg="Error:".mysqli_error($conn);// display error message 
}
}
...
$sql="UPDATE livingroom SET type='".$type."', productname='".$name."', size='".$size."',price='".$price."',image='".$image."' where id=".$id.""; // update data in database livingroom table
	if(mysqli_query($conn,$sql))
...
{
$sql="DELETE FROM livingroom where id=".$id."";//delete data in database livingroom table
	if(mysqli_query($conn,$sql))
...
$sql="SELECT * FROM livingroom WHERE id='".$id."'"; //Select data from livingroom table
...
if (mysqli_num_rows ($result)>0){
	...
	echo "<b><center><img src=data:image;base64,". $row[5]." width= 120 height=120/><br> Type: ". $row [1]."<br> Name: ". $row [2]. " <br> Size: ". $row [3]. " <br> Price: ". $row [4]. "<b>";// code for show output
